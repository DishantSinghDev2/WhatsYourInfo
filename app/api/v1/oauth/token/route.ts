...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Determine the content type and parse the body accordingly
    if (contentType?.includes('application/json')) {
      body = await request.json();
    } else if (contentType?.includes('application/x-www-form-urlencoded')) {
      const formData = await request.formData();
      // Convert FormData to a plain JavaScript object to be used by Zod
      body = Object.fromEntries(formData.entries());
    } else {
      // Return an error if the content type is not supported
    ...
    // 2. The rest of the logic remains the same, now working with the unified 'body' object
    ...
    // Add more specific error handling for parsing failures
...
// --- NO CHANGES ARE NEEDED IN THE HANDLER FUNCTIONS BELOW ---

// Handler for the initial code-for-token exchange (unchanged)
...
// Handler for the refresh token exchange (unchanged)
...
// Helper to generate and store tokens (unchanged)
