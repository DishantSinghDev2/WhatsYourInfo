    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                router.push(`/login/verify-otp?view=classic&email=${formData.email}${(callbackUrl !== null) ? `&callbackUrl=${encodeURIComponent(callbackUrl)}` : ''}`);
            } else {
        ...
        ${formData.profileVisibility === 'private' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                            ...
                            <div className="mt-6">
                                ...
                                <div className="mt-6 text-center">
                                    <Link href={`/login?view=classic${callbackUrl && `&callbackUrl=${callbackUrl}`}`}><Button variant="outline" className="w-full">Sign In Instead</Button></Link>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    {/* Add a link to switch to the new view */}
                    <div className="text-center">
